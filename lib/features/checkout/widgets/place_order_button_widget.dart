...
import 'package:flutter_grocery/features/payment/screens/geidea_payment_screen.dart';
                ...
                if ((orderProvider.selectedPaymentMethod == null ? (orderProvider.selectedOfflineValue == null) : orderProvider.selectedPaymentMethod == null)) {
                  ...
                  if (placeOrderBody.paymentMethod == 'geidea_payment' || placeOrderBody.paymentMethod == 'geidea') {
                    ...
                    String url = "customer_id=${profileProvider.userInfoModel?.id ?? authProvider.getGuestId()}&&is_guest=${authProvider.getGuestId() != null ? '1' : '0'}"
                        "&&callback=${AppConstants.baseUrl}${RouteHelper.orderSuccessful}&&order_amount=${((checkOutData?.amount ?? 0) + (checkOutData?.deliveryCharge ?? 0)).toStringAsFixed(2)}";
                    String webUrl = "customer_id=${profileProvider.userInfoModel?.id ?? authProvider.getGuestId()}&&is_guest=${authProvider.getGuestId() != null ? '1' : '0'}"
                        "&&callback=$protocol//$hostname${kDebugMode ? ':$port' : ''}${'${RouteHelper.orderWebPayment}/get_way'}&&order_amount=${((checkOutData?.amount ?? 0) + (checkOutData?.deliveryCharge ?? 0)).toStringAsFixed(2)}&&status=";
