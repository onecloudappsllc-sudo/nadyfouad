                  ...
                  Expanded(flex: 6, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    if (orderProvider.addressIndex != -1) FutureBuilder<Map<String, dynamic>?>(
                      future: (locationProvider.addressList?[orderProvider.addressIndex] != null
                          && locationProvider.addressList![orderProvider.addressIndex].latitude != null
                          && locationProvider.addressList![orderProvider.addressIndex].latitude!.isNotEmpty
                          && locationProvider.addressList![orderProvider.addressIndex].longitude != null
                          && locationProvider.addressList![orderProvider.addressIndex].longitude!.isNotEmpty)
                      ...
                      builder: (context, snapshot) {
                       ...
                       try {
                            final allResults = snapshot.data!['results'] as List;
                            List<String> allNames = [];
                            for (var result in allResults) {
                              final comps = result['address_components'] as List;
                              for (var comp in comps) {
                                allNames.add((comp['long_name'] as String).toLowerCase());
                                if (comp['short_name'] != null) {
                                  allNames.add((comp['short_name'] as String).toLowerCase());
                                }
                              }
                              if (result['formatted_address'] != null) {
                                allNames.add((result['formatted_address'] as String).toLowerCase());
                              }
                            }
                            final areas = splashProvider.deliveryInfoModelList?[orderProvider.branchIndex].deliveryChargeByArea ?? [];
                            DeliveryChargeByArea? matchingArea;
                            int bestScore = 0;
                            // Filter out generic words that cause false matches
                            final skipWords = ['dubai', 'abu dhabi', 'sharjah', 'ajman', 'united arab emirates', 'uae'];
                            for (var area in areas) {
                              ...
                              for (var name in allNames) {
                                ...
                                if (nameTrimmed == areaName) {
                                  matchingArea = area;
                                  bestScore = 100;
                                  break;
                                }
                                // Skip generic city/country names for partial matching
                                if (skipWords.contains(nameTrimmed)) continue;
                                if (skipWords.contains(areaName)) continue;
                                // Address component contains area name
                                if (nameTrimmed.contains(areaName) && areaName.length > 3) {
                                  int score = areaName.length * 2;
                                  if (score > bestScore) {
                                    bestScore = score;
                                    matchingArea = area;
                                  }
                                }
                                // Area name contains address component
                                if (areaName.contains(nameTrimmed) && nameTrimmed.length > 3) {
                                  int score = nameTrimmed.length;
                                  if (score > bestScore) {
                                    bestScore = score;
                                    matchingArea = area;
                                  }
                                ...
                                if (orderProvider.selectedAreaID != matchingArea!.id) {
                                  showDialog(
                                    ...
                                    builder: (ctx) => AlertDialog(
                                      title: Text('تأكيد منطقة التوصيل', textAlign: TextAlign.center, style: poppinsSemiBold.copyWith(fontSize: Dimensions.fontSizeLarge)),
                                      content: Column(mainAxisSize: MainAxisSize.min, children: [
                                        Text('${matchingArea!.areaName}', style: poppinsMedium.copyWith(fontSize: Dimensions.fontSizeLarge, color: Theme.of(ctx).primaryColor), textAlign: TextAlign.center),
                                        const SizedBox(height: 8),
                                        Text('رسوم التوصيل: AED ${matchingArea!.deliveryCharge ?? 0}', style: poppinsSemiBold.copyWith(fontSize: 20, color: Theme.of(ctx).primaryColor), textAlign: TextAlign.center),
                                      ]),
                                      actions: [
                                        TextButton(onPressed: () => Navigator.pop(ctx), child: Text('اختيار يدوي', style: poppinsRegular.copyWith(color: Theme.of(ctx).hintColor))),
                                        ElevatedButton(
                                          style: ElevatedButton.styleFrom(backgroundColor: Theme.of(ctx).primaryColor),
                                          onPressed: () {
                                            orderProvider.setAreaID(areaID: matchingArea!.id!);
                                            double deliveryCharge = CheckOutHelper.getDeliveryCharge(freeDeliveryType: widget.freeDeliveryType, orderAmount: widget.amount, distance: orderProvider.distance, discount: widget.discount ?? 0, configModel: configModel);
                                            orderProvider.setDeliveryCharge(deliveryCharge);
                                            Navigator.pop(ctx);
                                          },
                                          child: Text('تأكيد', style: poppinsRegular.copyWith(color: Colors.white)),
                                        ),
                                      ],
                                    ),
                                  );
