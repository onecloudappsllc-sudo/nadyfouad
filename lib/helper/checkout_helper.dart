...
class CheckOutHelper {
  ...
  static selectDeliveryAddressAuto({AddressModel? lastAddress, required bool isLoggedIn, required String? orderType}) async {
    ...
    // Skip addresses without valid coordinates - find one with coordinates
    if(deliveryAddress != null && (deliveryAddress.latitude == null || deliveryAddress.latitude!.isEmpty)){
      if(locationProvider.addressList != null){
        for(int i = 0; i < locationProvider.addressList!.length; i++){
          final addr = locationProvider.addressList![i];
          if(addr != null && addr.latitude != null && addr.latitude!.isNotEmpty && addr.longitude != null && addr.longitude!.isNotEmpty){
            deliveryAddress = addr;
            break;
          }
        }
      }
      if(deliveryAddress!.latitude == null || deliveryAddress!.latitude!.isEmpty) return;
    }
  ...
  static String getDeliveryChargeType(){
    ...
    if(splashProvider.deliveryInfoModelList == null || splashProvider.deliveryInfoModelList!.isEmpty || orderProvider.branchIndex >= splashProvider.deliveryInfoModelList!.length){
      return '';
    }
